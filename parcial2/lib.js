...
function Run_Pyramid(){
    ...
    for (let level = 0; level < 15; level++){
        document.getElementById('pyramid').innerHTML +=`<div class=lvl id=lvl${level}></div>`;
        position+=level;
        for (let item = position; item <= level+position; item++){
            document.getElementById(`lvl${level}`).innerHTML +=`<span id=position${item}>${pyramid[item]}</span>`;
        }
        if(level==0)
            route[0]=0;
            else
                route[level]=Calculate_Route(route[level-1],level);
    }
}
function Calculate_Route(pos,lev){
    if (pyramid[pos+lev]>pyramid[pos+lev+1])
        r=pos+lev;
        else
            r=pos+lev+1;
        return r
